  ...
  <div class="container">
    ...
    <div class="skills-grid fade-in">
      {#each techStack as skill, i}
        ...
        >
          <div class="skill-row">
            <span class="skill-idx mono">{skill.idx}</span>
            <span class="skill-cat mono">{skill.categoryLabel}</span>
            <span class="skill-pct mono">{skill.pct}%</span>
          </div>
...
<style>
  ...
  .skill-idx { color: var(--fg-whisper); }
  .skill-pct { color: var(--accent-on); font-weight: 500; }
  .skill-name {
    font-family: var(--font-display);
    font-size: 1.2rem;
    font-weight: 600;
  ...
  .skill-fill {
    height: 100%;
    width: var(--pct);
    background: linear-gradient(92deg, var(--accent-on), color-mix(in oklch, var(--accent-on) 55%, white));
    border-radius: inherit;
    box-shadow: 0 0 10px var(--accent-on);
    transform-origin: left;
    animation: barFill 1200ms var(--ease-out-expo) both;
  }
